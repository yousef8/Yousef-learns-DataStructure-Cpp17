...
private:
    ...
    void delete_a_node(Node &cur, Node &prv)
    {
...
public:
    ...
    void remove_last_occurance(int key)
    {
        ...
        Node *prv_to_delete{nullptr};

        for (Node *cur{head}, *prv{nullptr}; cur; cur = cur->next)
        {
            if (cur->data == key)
                to_delete = cur, prv_to_delete = prv;
            prv = cur;
        }

        delete_a_node(*to_delete, *prv_to_delete);
        debug_verify_data_integrity();
        return;
    }
};
